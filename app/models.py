...
class VisitRecord(db.Model):
    ...
    recommand = db.Column(db.Interger, unique=False, nullable=False, comment='是否值得推薦其他同學租賃')
    
    # 賃居安全自主管理檢視資料（學生填寫）
    safe_manage1 = db.Column(db.Interger, unique=False, nullable=False, comment='木造隔間或鐵皮加蓋')
    safe_manage2 = db.Column(db.Interger, unique=False, nullable=False, comment='有火警警報器或偵煙器')
    safe_manage3 = db.Column(db.Interger, unique=False, nullable=False, comment='逃生通道暢通且標示清楚')
    safe_manage4 = db.Column(db.Interger, unique=False, nullable=False, comment='門禁及鎖具良好管理')
    safe_manage5 = db.Column(db.Interger, unique=False, nullable=False, comment='有安裝照明設備(停車場及周邊)')
    safe_manage6 = db.Column(db.Interger, unique=False, nullable=False, comment='瞭解熟悉電路安全及逃生要領')
    safe_manage7 = db.Column(db.Interger, unique=False, nullable=False, comment='熟悉派出所、醫療、消防隊、學校校安專線電話')
    safe_manage8 = db.Column(db.Interger, unique=False, nullable=False, comment='使用多種電器(高耗能)，是否同時插在同一條延長線')
    safe_manage9 = db.Column(db.Interger, unique=False, nullable=False, comment='有滅火器且功能正常')
    safe_manage10 = db.Column(db.Interger, unique=False, nullable=False, comment='熱水器(電熱式及瓦斯式)安全良好，無一氧化碳中毒疑慮')
    safe_manage11 = db.Column(db.Interger, unique=False, nullable=False, comment='分間6個以上房間或10個以上床位')
    safe_manage12 = db.Column(db.Interger, unique=False, nullable=False, comment='有安裝監視器設備')
    safe_manage13 = db.Column(db.Interger, unique=False, nullable=False, comment='使用<內政部定型化租賃契約>')

    # 環境作息及評估（導師填寫）
    deposit_demand = db.Column(db.Interger, unique=False, nullable=False, comment='押金要求。 True:合理 False:不合理')
    water_electric_bill_demand = db.Column(db.Interger, unique=False, nullable=False, comment='水電費要求。 True:合理 False:不合理')
    ...
    is_get_along_with = db.Column(db.Interger, unique=False, nullable=False, comment='主客相處。 True:和睦 False:欠佳')
    ...
    traffic_safty = db.Column(db.Interger, unique=False, nullable=False, comment='交通安全')
    smoke = db.Column(db.Interger, unique=False, nullable=False, comment='拒絕菸害')
    drug = db.Column(db.Interger, unique=False, nullable=False, comment='拒絕毒品')
    dengue = db.Column(db.Interger, unique=False, nullable=False, comment='登革熱防治')
